...
C4R_to_CAVA <- function(obs = NULL,
  ...
  check_inputs.c4r(projections, historical,obs)

  country_shp <- geo_localize.c4r(projections, historical)
...
#' @noRd
check_inputs.c4r <- function(projections, historical, obs) {
...
#' Check validity of function arguments
#' @noRd
geo_localize.c4r <- function(projections, historical) {
  if (!is.null(projections) | !is.null(historical))  {
    coords <-
      if (!is.null(historical))
