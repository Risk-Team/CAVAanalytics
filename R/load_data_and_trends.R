...
load_data_and_trends <- function(variable,
  ...
  for (i in 1:(length(x_chunks) - 1)) {
    for (j in 1:(length(y_chunks) - 1)) {
      ...
      cli::cli_progress_step(paste(
        paste0(
          "Loading data and applying trends function to spatial CHUNK_",
          i,
          "_",
          j
        ),
  ...
  invisible(structure(
    list(
      rasters_coef %>% terra::crop(., country_shp) %>% terra::mask(., country_shp),
      rasters_pval %>% terra::crop(., country_shp) %>% terra::mask(., country_shp),
      rasters_mbrs_coef %>% terra::crop(., country_shp) %>% terra::mask(., country_shp),
      rasters_mbrs_pval %>% terra::crop(., country_shp) %>% terra::mask(., country_shp),
      NULL
    ),
