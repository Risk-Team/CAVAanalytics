...
In these trend maps, the overlaid marker indicates **no statistical significance** (i.e., `p >= 0.05`), while pixels without marker are significant.
...
```{r, fig.height=12, fig.width=10,  message=F, warning=F}
...
As an alternative to performing bias correction locally, CAVAanalytics now provides access to pre-computed bias-corrected CORDEX-CORE simulations. These datasets have been bias-corrected using ERA5 as the reference dataset following the ISIMIP methodology, and are currently available for the AFR-22 and WAS-22 domains (with additional domains to be released soon).

To load these pre-computed bias-corrected datasets, simply use `path.to.data = "CORDEX-CORE-BC"` instead of `path.to.data = "CORDEX-CORE"`. **We also load the ERA5 observations to verify the bias correction.**
...
We can now verify that the data is bias-corrected by comparing it against the observations for  the number of very warm days (days with maximum temperature above 35 °C) as above.
...
As shown above, the biases are more negligible than in the non bias-corrected data. This means you can directly use the data for projections and climate change signal analysis without needing to apply bias correction:

Since the data is already bias-corrected, you should **not** set `bias.correction = TRUE` when using functions like `projections()` or `climate_change_signal()` with these datasets.

In this specific case, it seems that the eqm method is working better than the ISIMIP3b method. 
...
For climate change signal maps, the overlaid marker/hatching indicates **lack of agreement** among models (agreement below the selected threshold).
