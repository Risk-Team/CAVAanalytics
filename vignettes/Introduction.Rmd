...
To load these pre-computed bias-corrected datasets, simply use `path.to.data = "CORDEX-CORE-BC"` instead of `path.to.data = "CORDEX-CORE"`:
...
years.proj = 2030:2055, years.hist = 1980:2005, domain = "AFR-22")
```

The data structure is identical to the standard CORDEX-CORE data, but the models are already bias-corrected. This means you can directly use the data for projections and climate change signal analysis without needing to apply bias correction:

```{r, fig.height=4, fig.width=5, fig.align="center", message=F, warning=F}
# Directly calculate climate change signal with pre-computed bias-corrected data
ccs_kenya_bc <- climate_change_signal(kenya_bc, season=list(1:12), threshold = 0.8)

plotting(ccs_kenya_bc, plot_titles = "Δ °C (bias-corrected",legend_range = c(-3, 3), 
