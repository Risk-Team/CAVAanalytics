...
What if we want to understand the projected total duration of heat waves **with at least 3 consecutive days** with maximum temperature above 35 degrees. This can be done by specifying the threshold (uppert), with consecutive = TRUE and duration=3.
...
What if we want to also understand the projected maximum length of a heat wave, defined as consecutive number of days with maximum temperature above 35 degrees. This can be done by specifying the threshold (uppert), with consecutive = TRUE and duration="max".
...
The same concept applies to all other functions available in CAVAanalytics step 2 and variables.
